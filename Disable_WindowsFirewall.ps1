...
{ 
    ...
    if ($firewallstatus.Enabled -eq 'False') {
        Write-Host "Windows Firewall is disabled. No action required."
    }
    elseif ($firewallstatus.Enabled -eq 'True') {
        Write-Host "Windows firewall is enabled. Disabling ..."   
    }
        try {
...
$verify = Get-NetFirewallProfile | Select-Object Name,Enabled | Write-Host